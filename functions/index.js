...
async function claimNotification(eventKey, uid, payment) {
  ...
  const notificationRef = database.ref(
    `/system/telegramPaymentNotifications/${hashKey(eventKey)}`
  );
  ...
  const result = await notificationRef.transaction((current) => {
    ...
    if (
      ...
      now - Number(current.startedAt || 0) < CLAIM_TIMEOUT_MS
    ) {
      return current;
    }
  ...
  return {
    ref: notificationRef,
    claimed: result.committed && claimed
  };
}
...
exports.notifyOnPendingPayment = onValueWritten(
  ...
  async (event) => {
    ...
    if (
      beforeWasPending &&
      paymentEventKey(uid, before) === paymentEventKey(uid, payment)
    ) {
      return;
    }
    ...
    try {
      const message = formatPaymentMessage(uid, profile, payment);
      ...
      console.log(
        `Telegram payment notification sent for user ${uid}.`
      );
    } catch (error) {
      await claim.ref.update({
        status: "failed",
